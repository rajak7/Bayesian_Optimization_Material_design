      ...
      do
		...
		fourth=${M4}${X4}
		    ...
		    symm_index=$((symm_index+5))
               ...
               then
                   symm_index=$((symm_index+216))
               elif [[ $fourth == MoTe ]]
               then
                   symm_index=$((symm_index+432))
               elif [[ $fourth == WS ]]
               then
                   symm_index=$((symm_index+648))
               elif [[ $fourth == WSe ]]
               then
                   symm_index=$((symm_index+864))
               else
                   symm_index=$((symm_index+1080))
               fi
               if (( $i > $symm_index ))
               then
                echo "already submitted "$M1${X1}2-$M2${X2}2-$M3${X3}2-$M4${X4}2
               else
                echo "submitting "$M1${X1}2-$M2${X2}2-$M3${X3}2-$M4${X4}2
                submitted=$((submitted+1))
                #python snl_prep.py -s $M1${X1}2-$M2${X2}2-$M3${X3}2-$M4${X4}2 -d .
               fi
               i=$((i+1))
              done
            done
